...
void decomp(double *A, double *U, double *L, int n);
...
void decomp(double *A, double *U, double *L, int n){
    ...
    nproc = calculateBlock(world_size, n);
    nblock = n / nproc;
    all_proc = nproc * nproc;
    int world_color = (world_rank < all_proc) ? 0 : 1;

    int square_rank, square_size;
    MPI_Comm square_comm;
    MPI_Comm_split(MPI_COMM_WORLD, world_color, world_rank, &square_comm);
    MPI_Comm_rank(square_comm, &square_rank);
    MPI_Comm_size(square_comm, &square_size);
    ...
    if(world_color == 0){
        ...
        MPI_Comm_split(square_comm, diag_color, square_rank, &root_comm);
        MPI_Comm_rank(root_comm, &diag_rank);
        
        for(int m = 0; m < nproc; ++m){
            int sub_color = (coords[0] >= m && coords[1] >= m) ? 0 : 1;
            MPI_Comm_split(square_comm, sub_color, square_rank, &sub_comm);
            if(sub_color == 0){

                MPI_Comm_rank(sub_comm, &sub_rank);
                MPI_Comm_size(sub_comm, &sub_size);
                MPI_Cart_create(sub_comm, 2, nprocdim, period, 1, &sub_cart_comm);
                MPI_Cart_coords(sub_cart_comm, sub_rank, 2, sub_coords);
                ...
                if(sub_rank == 0){
                    //if(m == 0) printf("nblock %d nproc %d subsize %d\n", nblock, nproc, sub_size);
                    for(int i = 0; i < sub_size; ++i){
                        sendcounts[i] = 1;
                    }
                    int displ = m * (nblock * nproc + 1), pdispl;
                    for(int i = 0; i < nproc - m; ++i){
                        pdispl = displ;
                        for(int j = 0; j < nproc - m; ++j, ++displ){
                            displs[i*(nproc - m) + j] = displ; 
                        }
                        displ = pdispl + nblock * nproc;
                    }
                    // for(int i = 0; i < sub_size; ++i){
                    //     printf("%d ", displs[i]);
                    // }
                ...
                MPI_Scatterv(A, sendcounts, displs, sub_array_t, subA, nblock * nblock, MPI_DOUBLE, ROOT, sub_comm);
                ...
                if(sub_rank == ROOT){
                    ...
                    // if(m == 0){
                    //     matrix_print(subU, nblock);
                    //     matrix_print(subL, nblock);
                    // }
                }
                ...
                if(sub_size != 1){
                    ...
                    if(row_color != 0 && col_color != 0){
                        ...
                        //if(m == 0) matrix_print(subA, nblock);
                    }
                    MPI_Comm_free(&col_comm);
                    MPI_Comm_free(&row_comm);
                }

                MPI_Gatherv(subA, nblock * nblock, MPI_DOUBLE, A, sendcounts, displs, sub_array_t, ROOT, sub_comm);
                MPI_Gatherv(subL, nblock * nblock, MPI_DOUBLE, L, sendcounts, displs, sub_array_t, ROOT, sub_comm);
                MPI_Gatherv(subU, nblock * nblock, MPI_DOUBLE, U, sendcounts, displs, sub_array_t, ROOT, sub_comm);
                //if(m == 0 && sub_rank == ROOT) matrix_print(U, n);
